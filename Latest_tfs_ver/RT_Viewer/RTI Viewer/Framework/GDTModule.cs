...
namespace RT_Viewer.Framework
{
    public class GDTModule : IModule
    {
        ...
        public override bool update(byte[] msg, uint size, uint type, uint source)
        {
            ...
            try
            {
                ...
                for (int Index = 0; Index < GDT_TABLE_SIZE; Index++)
                {
                    for (int j = 0; j < (int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_MAX; j++)
                    {
                        ...
                        indexReader += 4;
                        gdtStruct.channels[Index].rx_control_status[j].cntr_ok = (uint)BitConverter.ToUInt32(msg, indexReader);
                        indexReader += 4;
                        gdtStruct.channels[Index].rx_control_status[j].cntr_err = (uint)BitConverter.ToUInt32(msg, indexReader);
                        indexReader += 4;
                    }
                    for (int j = 0; j < (int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_MAX; j++)
                    {
                        ...
                        indexReader += 4;
                        gdtStruct.channels[Index].tx_gdt[j].cntr_ok = (uint)BitConverter.ToUInt32(msg, indexReader);
                        indexReader += 4;
                        gdtStruct.channels[Index].tx_gdt[j].cntr_err = (uint)BitConverter.ToUInt32(msg, indexReader);
                        indexReader += 4;
                    }
                }
            ...
            if (gdtStruct.size >= GDT_TABLE_SIZE)
            {
                Console.WriteLine("gdtStruct.size is too big : " +gdtStruct.size.ToString());
            }
            else
            {
                ...
                for (int i = 0; i < gdtStruct.size; i++)
                {
                    ...
                    BL_GDTTable.Add(new GDTTable
                    {
                        ...
                        Rx_CBand = getIPStringFromUint(gdtStruct.channels[i].rx_control_status[(int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_C_BAND].ip) + ":" +
                                                            gdtStruct.channels[i].rx_control_status[(int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_C_BAND].port.ToString() + " / " +
                                                                gdtStruct.channels[i].rx_control_status[(int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_C_BAND].cntr_ok.ToString(),
                        Rx_UHF = getIPStringFromUint(gdtStruct.channels[i].rx_control_status[(int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_UHF].ip) + ":" +
                                                            gdtStruct.channels[i].rx_control_status[(int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_UHF].port.ToString() + " / " +
                                                                gdtStruct.channels[i].rx_control_status[(int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_UHF].cntr_ok.ToString(),
                        Tx_CBand = getIPStringFromUint(gdtStruct.channels[i].tx_gdt[(int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_C_BAND].ip) + ":" +
                                                            gdtStruct.channels[i].tx_gdt[(int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_C_BAND].port.ToString() + " / " +
                                                                gdtStruct.channels[i].rx_control_status[(int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_C_BAND].cntr_ok.ToString(),
                        Tx_UHF = getIPStringFromUint(gdtStruct.channels[i].tx_gdt[(int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_UHF].ip) + ":" +
                                                            gdtStruct.channels[i].tx_gdt[(int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_UHF].port.ToString() + " / " +
                                                                gdtStruct.channels[i].rx_control_status[(int)enumAPP_gdt_commType.APP_COMMON_GDT_COMM_TYPE_UHF].cntr_ok.ToString(),
            ...
            {
                Console.WriteLine("Bad data for GDT :" + ex.Message);
            }
